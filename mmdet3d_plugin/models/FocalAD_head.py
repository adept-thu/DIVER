...
class FocalADHead(BaseModule):
    def __init__(
        ...
        **kwargs,
        ...
        super(FocalADHead, self).__init__(init_cfg)
    ...
    def forward(
        ...
        if self.task_config['with_motion_plan']:
            motion_output, planning_output, topk_idx, alpha = self.motion_plan_head(
                det_output, 
                map_output, 
                feature_maps,
                metas,
        ...
        return det_output, map_output, motion_output, planning_output, topk_idx, alpha

    def loss(self, model_outs, data):
        det_output, map_output, motion_output, planning_output, topk_idx, alpha= model_outs
        losses = dict()
        if self.task_config['with_det']:
