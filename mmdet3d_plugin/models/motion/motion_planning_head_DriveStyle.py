...
try:
    from mmdet3d_plugin.ops import deformable_aggregation_function as DAF
except:
    DAF = None
from mmdet3d_plugin.models.motion.modules.conditional_unet1d import ConditionalUnet1D, SinusoidalPosEmb
import torch.nn.functional as F
...
class DriveStyleMotionPlanningHead(BaseModule):
    ...
    def check_collision(self, trajectories, agents, safe_distance=2.0):
        ...
        safe = True
        for agent in agents:
            min_dist = torch.min(torch.norm(trajectories - agent, dim=-1))
    ...
    def forward(
        ...
        #instance_feature torch.Size([6, 900, 256])
        #anchor_embed torch.Size([6, 900, 256])
        #det_classification torch.Size([6, 900, 9])
        #det_anchors torch.Size([6, 900, 11])
        #det_confidence torch.Size([6, 900])
        #instance_feature_selected torch.Size([6, 50, 256])
        #anchor_embed_selected torch.Size([6, 50, 256])
        #det_confidence torch.Size([6, 900])
        #self.num_det 50
        import pdb; pdb.set_trace()
        ...
        # map_instance_feature_selected [6，10，256]
        # map_anchor_embed_selected[6，10，256]
        # map_confidence [6，100]
        # num_map 10
        # map_instance_feature [6,100,256]
        # 
        ...
        #
        # 只在cmd内加噪声，6次，预测出的噪声直接编码
        # predicted和traj 分别单独搞个模型参数
        # =================== forward the layers ====================
        ...
        for i, op in enumerate(self.operation_order):
            if self.layers[i] is None:
                ...
                instance_feature = self.graph_model(  #instance_feature torch.Size([2, 901, 256])
                    i,
                    instance_feature.flatten(0, 1).unsqueeze(1),#torch.Size([1802, 1, 256])
                    temp_instance_feature,#torch.Size([1802, 1, 256])
                    temp_instance_feature,#torch.Size([1802, 1, 256])
                    query_pos=anchor_embed.flatten(0, 1).unsqueeze(1),#torch.Size([1802, 1, 256])
                    key_pos=temp_anchor_embed,#torch.Size([1802, 1, 256])
                    key_padding_mask=temp_mask,#torch.Size([1802, 1])
                )
                instance_feature = instance_feature.reshape(bs, num_anchor + 1, dim)#torch.Size([1802, 1, 256])
            elif op == "gnn":
                instance_feature = self.graph_model(
                    i,
                    instance_feature,#instance_feature torch.Size([2, 901, 256])
                    instance_feature_selected,#torch.Size([2, 51, 256])
                    instance_feature_selected,#torch.Size([2, 51, 256])
                    query_pos=anchor_embed,#torch.Size([2, 901, 256])
                    key_pos=anchor_embed_selected,#torch.Size([2, 51, 256])
                )
            elif op == "norm" or op == "ffn":
                ...
                plan_query = plan_mode_query + (instance_feature + anchor_embed)[:, num_anchor:].unsqueeze(2)#torch.Size([2, 1, 6, 256])
                ...
                planning_classification.append(plan_cls)#torch.Size([6, 1, 6])
                planning_prediction.append(plan_reg)#torch.Size([6, 1, 6, 6, 2])
                # import pdb; pdb.set_trace()
        ...
        predicted_noise_tensor_cmd = predicted_noise_tensor.view(bs,
            ...
            self.ego_fut_mode,
            self.ego_fut_ts,
            predicted_noise_tensor.shape[3])#（预测的多模态轨迹）
        #import pdb;pdb.set_trace()
        ...
        plan_nav_query = plan_nav_query.view(bs,6,6,-1)#torch.Size([6, 6, 6, 256])
            ...
            for m in range(num_modes):
                ...
                for a in range(other_agents[b].shape[0]):
                    ...
                    if valid_mask.sum() == 0:
                        continue
                    # 有效帧距离
                    dists = torch.norm(ego_traj - other_traj, dim=1)  # [T]
                    collision = (dists < safe_distance) & valid_mask
                    if collision.any():
    ...
    def diff_PPO_loss_planning(self, model_outs, data):
        ...
        for decoder_idx, (cls, reg) in enumerate(zip(cls_scores, reg_preds)):
            ...
            output.update({
                f"diff_PPO_planning_loss_cls_{decoder_idx}": cls_loss,
                f"diff_PPO_planning_loss_reg_{decoder_idx}": reg_loss,
                f"diff_PPO_loss_{decoder_idx}": diffusion_loss,
                f"diff_PPO_reward_total_{decoder_idx}": total_reward.mean().detach(),
            })
