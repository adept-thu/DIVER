...
#import pdb;pdb.set_trace()
clusters = []
intention_car = np.concatenate(intention[0], axis=0).reshape(-1, 12)
kmeans = KMeans(n_clusters=100)
kmeans.fit(intention_car)
...
for j in range(100):
    ...
    intention_car_sample = intention_car[sampled_index].reshape(-1, 6, 2)
    #import pdb;pdb.set_trace()
    plt.plot(intention_car_sample[0 ,:, 0], 
            intention_car_sample[0, :,1], 
            markersize=10, 
            linewidth=2.5,
            marker='o', 
            color= color[j % 7]
    ...
    plt.grid(True)
    plt.xticks(np.arange(0, 80, 1))  # x轴从0到10，步长0.5
    plt.yticks(np.arange(-40, 40, 1))
    #plt.scatter(intention_car_sample[0 ,:, 0], intention_car_sample[0, :,1])
    plt.savefig(f'/data/songziying/workspace/SparseDriveb2d/motion_sample/vis{j}.png', bbox_inches='tight')
    plt.close()
import pdb;pdb.set_trace()
for i in range(len(CLASSES)):
